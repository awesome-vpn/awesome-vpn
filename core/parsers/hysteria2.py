...
def parse(data):
    ...
    node = {
        ...
        "server_port": int(
            re.search(r"\d+", server_info.netloc.rsplit(":", 1)[-1].split(",")[0]).group()
        ),
        "password": netquery["auth"]
        if netquery.get("auth")
        else server_info.netloc.split("@")[0].rsplit(":", 1)[-1],
    ...
    if str(netquery.get("insecure", "")).lower() in ("1", "true") or str(
        netquery.get("allowInsecure", "")
    ).lower() in ("1", "true"):
