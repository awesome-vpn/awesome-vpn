...
def clash2v2ray(share_link):
    ...
    if share_link["type"] == "vmess":
        ...
        base_link = base64.b64encode(
            "{cipher}:{password}".format(**ss_info).encode("utf-8")
        ).decode("utf-8")
        if share_link.get("plugin"):
            ...
            if share_link.get("plugin") == "obfs" or share_link.get("plugin") == "obfs-local":
                ...
                url_link = "?plugin=obfs-local%3Bobfs%3D{mode}%3Bobfs-host%3D{host}".format(
                    **ss_info
                )
        ...
        ssr_info = {
            ...
            "password": base64.b64encode(share_link.get("password", "").encode("utf-8")).decode(
                "utf-8"
            ),
        ...
        if trojan_info["type"] == "grpc":
            if share_link.get("grpc-opts", {}).get("grpc-service-name", "") not in [
                ...
                trojan_info["serviceName"] = unquote(
                    share_link.get("grpc-opts").get("grpc-service-name")
                )
            else:
            ...
            if share_link.get("ws-opts"):
                ...
                trojan_info["host"] = (
                    share_link.get("ws-opts", {}).get("headers", {}).get("Host", "")
                )
            else:
                trojan_info["path"] = ""
        ...
        if vless_info["type"] == "ws":
            ...
            if share_link.get("grpc-opts", {}).get("grpc-service-name", "") not in ["/", ""]:
                vless_info["serviceName"] = unquote(
                    share_link.get("grpc-opts").get("grpc-service-name")
                )
            else:
        ...
        if share_link.get("username"):
            ...
            base_link = base64.b64encode(
                "{server}:{port}".format(**http_info).encode("utf-8")
            ).decode("utf-8")
        ...
        if share_link.get("username"):
            ...
            base_link = base64.b64encode(
                "{server}:{port}".format(**socks5_info).encode("utf-8")
            ).decode("utf-8")
