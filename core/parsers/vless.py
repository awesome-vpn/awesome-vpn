...
def parse(data):
    ...
    if netquery.get("security", "") not in ["None", "none", ""] or netquery.get("tls") == "1":
        node["tls"] = {"enabled": True, "insecure": False, "server_name": ""}
        if str(netquery.get("allowInsecure", "")).lower() in ("1", "true"):
        ...
        if (netquery.get("security") == "reality" or pbk) and pbk:  # shadowrocket
            ...
            if netquery.get("fp"):
                node["tls"]["utls"] = {"enabled": True, "fingerprint": netquery["fp"]}
    if netquery.get("type"):
        if netquery["type"] == "http":
            ...
            node["transport"] = {
                ...
                if matches
                else netquery.get("path", "/"),
                "headers": {
                    "Host": ""
                    if netquery.get("host") is None and netquery.get("sni") == "None"
            ...
            node["transport"] = {"type": "grpc", "service_name": netquery.get("serviceName", "")}
        ...
        if netquery["obfs"] == "websocket":
            ...
            node["transport"] = {
                ...
                "headers": {
                    "Host": ""
                    if netquery.get("obfsParam") is None and netquery.get("sni") == "None"
    ...
    if netquery.get("protocol") in ["smux", "yamux", "h2mux"]:
        node["multiplex"] = {"enabled": True, "protocol": netquery["protocol"]}
