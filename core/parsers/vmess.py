...
def parse(data):
    ...
    try:
        if info.find("?") > -1:  # fuck奇葩的URI格式
            ...
            if (netquery.get("tls") and netquery["tls"] != "") or (
                ...
                allow_insecure = str(netquery.get("allowInsecure", "")).strip().lower()
                if allow_insecure in {"0", "false"}:
            ...
            if (netquery.get("obfs") == "websocket") or (netquery.get("type") == "ws"):
                ...
                try:
                    ...
                    node["transport"]["headers"]["Host"] = host_from_obfs_param or netquery.get(
                        "host", ""
                    )
    ...
    if "tls" in item and (item["tls"] != "" and item["tls"] != "none"):
        ...
        if item.get("fp"):
            node["tls"]["utls"] = {"enabled": True, "fingerprint": item["fp"]}
    if item.get("net"):
        if item["net"] in ["h2", "http", "tcp"]:
            ...
            if item.get("host"):
                node["transport"] = {"type": "ws", "headers": {"Host": item["host"]}}
            if item.get("path"):
                matches = re.search(r"\?ed=(\d+)$", item["path"])
                node["transport"]["path"] = (
                    item["path"].rsplit("?ed=", 1)[0] if matches else item["path"]
                )
                if matches:
            ...
            node["transport"] = {"type": "grpc", "service_name": item.get("path", "")}
    if item.get("protocol") in ["smux", "yamux", "h2mux"]:
        node["multiplex"] = {"enabled": True, "protocol": item["protocol"]}
