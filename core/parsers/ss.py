...
def parse(data):
    ...
    if param.find("plugin=obfs-local") > -1 or param.find("plugin=simple-obfs") > -1:
        if param.find("&", param.find("plugin")) > -1:
            plugin = urllib.parse.unquote(
                param[param.find("plugin") : param.find("&", param.find("plugin"))]
            )
        else:
        ...
        result_str = "obfs={};{}".format(
            ...
            "obfs-host={};".format(plugin_dict["obfs-host"])
            if plugin_dict.get("obfs-host")
            else "",
        )
        ...
        if param.find("&", param.find("v2ray-plugin")) > -1:
            try:
                plugin = tool.b64Decode(
                    param[
                        param.find("v2ray-plugin") + 13 : param.find(
                            "&", param.find("v2ray-plugin")
                        )
                    ]
                ).decode("utf-8")
            except Exception:
                plugin = urllib.parse.unquote(
                    param[
                        param.find("v2ray-plugin") + 15 : param.find(
                            "&", param.find("v2ray-plugin")
                        )
                    ]
                )
    ...
    if data[5:].find("protocol") > -1:
        ...
        node["multiplex"] = {"enabled": True, "protocol": smux_dict["protocol"]}
    ...
    if param2.find("shadow-tls") > -1:
        ...
        if param2.find("&", param2.find("shadow-tls")) > -1:
            plugin = tool.b64Decode(
                param2[
                    param2.find("shadow-tls") + 11 : param2.find("&", param2.find("shadow-tls"))
                ].split("#")[0]
            ).decode("utf-8")
        else:
            plugin = tool.b64Decode(param2[param2.find("shadow-tls") + 11 :].split("#")[0]).decode(
                "utf-8"
            )
        ...
        node_tls = {
            ...
            "tls": {"enabled": True, "server_name": plugin.get("host", "")},
        }
        ...
        if plugin.get("fp"):
            node_tls["tls"]["utls"] = {"enabled": True, "fingerprint": plugin.get("fp")}
        del node["server"]
